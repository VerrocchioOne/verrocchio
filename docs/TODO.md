...
>
> **🟢 Increment / multi-slot conflict resolved 2026-05-15** — The two modes were semantically incompatible (per-slot binary vs cumulative-per-day count). Rather than try to compose them, the Edit Habit modal now enforces mutual exclusion in the UI: see [index.html:26953-26999](../index.html#L26953-L26999) (Yes/No vs Target & Unit buttons get `disabled` + `not-allowed` cursor + 55% opacity + tooltip when `eHSlotSections.length >= 2`) and [index.html:27259-27302](../index.html#L27259-L27302) (multi-slot pills get the same treatment when `eHHasTarget` is true, plus an inline "— disabled while Target & Unit is on" hint next to the section label). Legacy habits with both flags continue to render as multi-slot (the increment + button is never drawn for slot rows, so the unreachable branch never fires from the renderer), so no data migration was needed. The (x/y) progress badge ([§5.7](#57-visual-clarity--distinguish-layered-vs-grouped-habits)) covers the "2 of 3 today" surface for the multi-slot case.
>
> **Still possibly broken** (not yet repro'd or fixed):
> - Partial-day rollup is not visually surfaced for grouped (non-multi-slot) habits — only multi-slot now shows (x/y). The grouped case is tracked under [§5.7](#57-visual-clarity--distinguish-layered-vs-grouped-habits).
> - The forward-looking model in this section (per-slot quantity targets, optional time-bound slots, streak rules for partial days) is not yet implemented. The current model is binary per slot. Composing the two properly would replace the mutual-exclusion lock with a richer data shape; until users actually ask for it, mutual exclusion is the right call.

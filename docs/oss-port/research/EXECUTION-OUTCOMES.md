...
| **§13.3g (v71)** | **(SortableJS removed)** | (pending commit) | — | **structural fix** | **DRAG-AND-DROP DELETED ENTIRELY.** After 3+ sessions of repeated regressions across hand-rolled pointer pipelines + SortableJS + the §13.3f layered-drop restoration, the user reported the reorder still felt broken. Per DevMoses: stop fixing the output, replace the failing process. SortableJS CDN tag deleted (the entire 11 KB gzipped dependency is out of the bundle). The 258-line `§13.3` SortableJS init useEffect deleted. `sortableDiag` state + diag bar deleted. Drop-preview indicator render deleted. Drag-only card render props (isBeingDragged, dragOffsetY, hrow-dragging className, touchAction:none, cursor:grab) deleted. **Replacement (~85 lines):** three buttons on each top-level habit card in reorder mode — ▲ moves up one position, ▼ moves down one, ⇶ toggles concurrent-with-above. Pure click handlers that call new helpers (`habitPositionInSection`, `moveHabitWithinSection`, `toggleConcurrentForHabit`) which mutate `data` via `save()` with no DOM coordinates, no pointer events, no library state, no iOS Safari quirks. Cohort math (position-0 invariant, cohort-starter flips) reuses the existing `commitHabitReorderDrop` function — same logic, different call site. Suite stayed at 21/21 desktop E2E + 124/124 utils throughout. Net delta on this change: **~−240 LOC** + 1 CDN dep removed. The entire class of recurring reorder bug is now structurally impossible because the failing primitive no longer exists in the code path. |

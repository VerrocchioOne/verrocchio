...
function HabitsView(props) {
  ...
  // ─── Touch drag-and-drop (iOS organize tool, see HabitsDragController) ─
  // On touch devices, reorder mode renders drag handles + hides the
  // toolbar; on desktop the toolbar stays as the primary path. Refs
  // close over the LATEST groupedH / commit callback so the once-
  // created controller always sees current state.
  const isTouchDevice = deviceProfile === "phone" || deviceProfile === "tablet";
  const [dragState, setDragState] = React.useState(null);
  const dragStateRef = React.useRef(null);
  dragStateRef.current = dragState;
  const enumerateSectionsRef = React.useRef(() => []);
  enumerateSectionsRef.current = () => (groupedH || []).map(g => ({
    section: g.value,
    orderedIds: (g.habits || []).filter(hh => !hh.parentId).map(hh => String(hh.id))
  }));
  const onCommitRef = React.useRef(() => {});
  const dragController = React.useMemo(() => {
    if (typeof window === "undefined" || !window.HabitsDragController) return null;
    return window.HabitsDragController.makeController({
      getDragState: () => dragStateRef.current,
      setDragState: (next) => setDragState(next),
      onCommit: (id, sec, idx, opts) => onCommitRef.current(id, sec, idx, opts),
      enumerateSections: () => enumerateSectionsRef.current()
    });
  }, []);
  ...
  onCommitRef.current = commitHabitReorderDrop;

  // Window pointer listeners while touch + reorder mode. passive:false
  // on pointermove so we can preventDefault iOS Safari scroll;
  // pointercancel aborts cleanly if iOS interrupts the gesture.
  React.useEffect(() => {
    if (!dragController || !reorderMode || !isTouchDevice) return undefined;
    const onMove = (e) => dragController.onPointerMove(e);
    const onUp = (e) => dragController.onPointerUp(e);
    const onCancel = (e) => dragController.onPointerCancel(e);
  ...
  }, [dragController, reorderMode, isTouchDevice]);
  ...
  // accompanying inline-source diff — see commit log for §13.4b.
    ...
    return /*#__PURE__*/React.createElement("div", {
      ...
      // Toolbar renders on DESKTOP only. Touch devices (phone/tablet)
      // use the drag-handle path instead — see the touch-exit bar
      // immediately below and the drag handle on each card.
      reorderMode && !isTouchDevice && window.HabitsReorderToolbar
        ...
        : null,
      // Touch reorder mode: slim sticky bar with the "Done" exit
      // affordance. Lives in HabitsTouchReorderBar.js to keep this
      // file under the 1000-LOC cap.
      reorderMode && isTouchDevice && window.HabitsTouchReorderBar &&
        /*#__PURE__*/React.createElement(window.HabitsTouchReorderBar, {
          dragState,
          onExitReorder: exitReorderMode
        }),
    ...
    /*#__PURE__*/React.createElement("div", {
      ...
      const renderCard = (window.HabitsHabitCard && window.HabitsHabitCard.makeRenderCard({
        ...
        // Touch drag-and-drop wiring (iOS organize tool). Card uses
        // dragController.beginDrag on the handle's onPointerDown when
        // BOTH dragController is non-null AND isTouchDevice is true
        // AND reorderMode is on. dragState.habitId is read to apply
        // a "being dragged" visual on the matching card.
        dragController, isTouchDevice, dragState
      })) || (() => null);

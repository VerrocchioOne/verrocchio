  ...
  function BriefView(props) {
    ...
    const _pastDays = (typeof window !== "undefined" && window.pastDays) || ((n) => {
      ...
      for (let i = 0; i < n; i++) {
        ...
        out.push((window.dk || (x => x.toISOString().slice(0, 10)))(d));
      }
      return out;
    });
    ...
    const today7 = (function () {
      ...
      for (let i = 0; i < 7; i++) {
        ...
        out.push(window.dk ? window.dk(d) : d.toISOString().slice(0, 10));
      }
      return out;
    })();
    ...
    const yMissedHabits = (function () {
      const missedDueStreak = (h0) => {
        ...
        for (let i = 1; i <= 60; i++) {
          const d = new Date(); d.setDate(d.getDate() - i);
          const k = window.dk ? window.dk(d) : d.toISOString().slice(0, 10);
    ...
    // ── Debrief entry cards (morning + evening) ──────────────────────
